...
router.get('/:projectId/:fileId', async (req: AuthRequest, res, next) => {
  await fileController.getFile(req, res, next);
});

router.put('/:projectId/:fileId', async (req: AuthRequest, res, next) => {
  await fileController.updateFile(req, res, next);
});

router.delete('/:projectId/:fileId', async (req: AuthRequest, res, next) => {
